...
Readonly our %INTEGER_TO_SYMBOL => (
    ...
    0x00000001 => 'STATUS_SEVERITY_INFORMATIONAL',
    0x00000002 => 'STATUS_SEVERITY_WARNING',
    0x00000003 => 'STATUS_SEVERITY_ERROR',
    ...
    0xC0150025 => 'STATUS_SXS_PRIMITIVE_INSTALLER_FAILED',
    0xC0150026 => 'STATUS_GENERIC_COMMAND_FAILED',
    0xC0150027 => 'STATUS_SXS_FILE_HASH_MISSING',
);

Readonly our %SYMBOL_TO_INTEGER => (
    # all the above, reversed ...
    (reverse %INTEGER_TO_SYMBOL),
    # ...and these duplicates, which were not included in INTEGER_TO_SYMBOL
...
# Directly export any values required by caller
our @EXPORT_OK = keys %SYMBOL_TO_INTEGER;

sub AUTOLOAD
{
    my $symbol = $Win32::Status::AUTOLOAD;
    return unless $symbol;

    # Win32::Status::FOO -> FOO
    my $pkg = quotemeta(__PACKAGE__);
    $symbol =~ s/^${pkg}:://;

    if (exists( $SYMBOL_TO_INTEGER{ $symbol } )) {
        return $SYMBOL_TO_INTEGER{ $symbol };
    }
    croak __PACKAGE__ . ": undefined symbol $symbol";
}
...
This module provides a perl interface to the symbols defined in the Windows
ntstatus.h header.  It allows mapping from integers to symbols and vice-versa.
